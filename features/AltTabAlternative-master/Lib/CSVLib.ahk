...
CSV_Load(FileName, CSV_Identifier="", Delimiter="`,")
{
  Local Row
  Local Col
  fileData :=  %CSV_Identifier%CSVFile
  FileRead, fileData, %FileName%
  ;~ FileAppend, [%fileData%]`n, *
...
;####################################################################################################################
CSV_Create(FileName, Cols, CSV_Identifier="", Delimiter="`,")
{
  Global
  fileData :=  %CSV_Identifier%CSVFile
  FileRead, fileData, %FileName%
  ...
  String := RegExReplace(String, RegExNeedle , "" )
  StringReplace,String, String,`r,@,All ;only for see msgbox 
  %CSV_Identifier%CSV_TotalRows := 0
  %CSV_Identifier%CSV_TotalCols := Cols
  %CSV_Identifier%CSV_Delimiter := Delimiter
  SplitPath, FileName, %CSV_Identifier%CSV_FileName, %CSV_Identifier%CSV_Path
  IfNotInString, FileName, `\
  {
...
;####################################################################################################################
CSV_MatchCellRow(CSV_Identifier, SearchText, RowNumber, Instance=1) ; fix http://www.autohotkey.com/forum/viewtopic.php?p=400669#400669
{
  Local Col
  Local FoundInstance
  CurrentCSV_TotalCols := %CSV_Identifier%CSV_TotalCols
